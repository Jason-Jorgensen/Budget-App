...
const investmentsSeed = [
  {
    "Current Retirement Balance": 50000,
    "Your 401K Contribution %": 7,
    "Employer 401K Contribution %": 3,
...
db.Debts
  .remove({})
  .then(() => db.Debts.collection.insertMany(debtsSeed))
  .then(data => {
    console.log(data.result.n + " records inserted!");
    process.exit(0);
  })
...
db.Investments
  .remove({})
  .then(() => db.Investments.collection.insertMany(investmentsSeed))
  .then(data => {
    console.log(data.result.n + " records inserted!");
    process.exit(0);
  })
